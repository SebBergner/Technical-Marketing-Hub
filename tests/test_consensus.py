...
def test_the_viewer_url_carries_the_sales_preview_parameter():
    ...
    somewhere dead. The mode itself flipped twice: `marketing` from
    2026-09-02 at Elio's request (dropping `sales`'s First Viewer / Second
    Viewer picker), back to `sales` on 2026-09-08 after Elio reviewed the Hub
    with Seb and reported `marketing` performing poorly.

    This test caught a real gap on the second flip: a value explicitly set in
    `.env` (`CONSENSUS_VIEWER_URL_TEMPLATE=...preview=marketing`) overrode the
    code default this test exercises, so the test kept passing against the
    OLD behaviour for a while, hiding an incomplete revert rather than
    catching it. `.env` is local-only and gitignored; the deployed instance's
    Azure App Setting of the same name needs checking by hand.
    """
    from backend.integrations.consensus_v2 import viewer_url
    url = viewer_url({"uuid": "abc-123"})
    assert url.endswith("?preview=sales"), url
    assert "preview=marketing" not in url
    assert "abc-123" in url


def test_a_marketing_preview_link_is_converted_back_to_sales():
    """Reverted 2026-09-08: Elio asked for `marketing` on 2026-09-02, then
    asked to go back after reviewing with Seb -- `marketing` "performed
    poorly" in practice. sales_view() is the same function as before with the
    direction flipped, not a new mechanism.
    """
    from backend.integrations.consensus_sync import sales_view

    assert sales_view("https://play.goconsensus.com/u1?preview=marketing")         == "https://play.goconsensus.com/u1?preview=sales"
    # Already sales, or some other mode a future API returns: left alone.
    assert sales_view("https://x/u1?preview=sales").endswith("sales")
    # No mode at all would open a viewer with nothing to play.
    assert sales_view("https://x/u1") == "https://x/u1?preview=sales"
    assert sales_view(None) is None
